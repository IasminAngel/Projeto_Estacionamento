...
document
  ...
  .addEventListener("submit", async function (event) {
    ...
    if (acao === "cadastrar") {
      ...
      const placaAlterar = prompt(
        "Confirme a placa do veículo que deseja alterar:"
      );
      if (!placaAlterar) return;

      try {
        const response = await fetch(
          `http://localhost:3306/alterar/${placaAlterar}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ cor, modelo, placa, tamanho }),
      ...
      const placaExcluir = prompt(
        "Confirme a placa do veículo que deseja excluir:"
      );
      if (!placaExcluir) return;

      try {
        const response = await fetch(
          `http://localhost:3306/excluir/${placaExcluir}`,
          {
            method: "DELETE",
          }
        );
...
// CODIGO ABAIXO SEM BANCO DE DADOS, PARA DEMONSTRAÇÃO EFETIVA EM TELA ! TIRE O COMENTÁRIO E COMENTE O CÓDIGO ACIMA !
